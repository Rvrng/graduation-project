...
from gpustack.runtime_state import request_finished
...
class ModelUsageMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        elif hasattr(request.state, "runtime_request_id"):
            await request_finished(
                getattr(request.state, "runtime_request_id", None),
                ok=False,
            )
...
async def process_request(
    ...
    if stream:
        ...
        try:
            ...
            prompt_tokens, completion_tokens = _usage_to_counts(usage)
        except Exception as e:
...
async def handle_streaming_response(
    ...
    async def streaming_generator():
        ...
        try:
            ...
            await request_finished(
                getattr(request.state, "runtime_request_id", None),
                ok=success and response.status_code < 400,
            )
    ...
    for line in lines[:-1]:
        ...
        if '"usage":' in data:
            ...
            if is_usage_chunk(response_chunk):
                ...
                prompt_tokens, completion_tokens = _usage_to_counts(response_chunk.usage)
                await request_finished(
                    getattr(request.state, "runtime_request_id", None),
                    prompt_tokens=prompt_tokens,
                    completion_tokens=completion_tokens,
                    ttft_ms=response_dict.get("usage", {}).get("time_to_first_token_ms"),
                    tokens_per_second=response_dict.get("usage", {}).get(
                        "tokens_per_second"
                    ),
                )
